...
namespace te {
	...
	//Конструктор, инициализируем root переданным значением
	//(при запуске терминала мы находимся в корневой директории)
	Terminal::Terminal(Directory* rootDirectory) :
		root(rootDirectory) {
		path.push_back(root);
	}
	...
	std::string Terminal::prompt() const {
		...
		//Проходим по всему пути и возвращаем имена
	...
	void Terminal::run() {
		...
		std::string input;
		std::cout << "\nText Editor Terminal. Type 'help' " << 
			"for commands name.\n";
			while (true) {
				std::cout << prompt();
				...
				//Сменить директорию
					...
					//Меняем директорию
						...
						//Ищем сущность в текущей директ.
						Entry* entry =
						cwd()->find(dir_name);
						if (!entry) {
							std::cout << "Directory: '" 
				...
				//Вывести содержимое файла
					...
					//Ищем дочернюю в текущей директории
					Entry* entry = cwd()->find(file_name);
